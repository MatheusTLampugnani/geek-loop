...
export default function ProductModal({ isOpen, product, onClose, onAddToCart }) {
  ...
  useEffect(() => {
    if (product) {
      ...
      setQuantity(1); setSelectedOption(null); setShowReviewForm(false); setFullScreenImage(null);
    ...
    <>
      {fullScreenImage && (
        <div className="fullscreen-image-overlay" onClick={() => setFullScreenImage(null)}>
          <button className="close-fullscreen-btn" onClick={() => setFullScreenImage(null)} title="Fechar foto">&times;</button>
          <img src={fullScreenImage} alt="Avaliação do cliente ampliada" className="fullscreen-image" />
        </div>
      )}
                                  ...
                                  {rev.imagem_url && (
                                      <div className="mt-2">
                                          <img 
                                              src={rev.imagem_url} 
                                              alt="Foto do Cliente" 
                                              className="customer-review-photo" 
                                              title="Clique para ampliar"
                                              onClick={() => setFullScreenImage(rev.imagem_url)}
                                          />
                                      </div>
                                  )}
                              </div>
